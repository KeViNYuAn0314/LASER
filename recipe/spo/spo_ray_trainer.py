...
class RayPPOTrainer:
    ...
    def _add_sample_ids_to_dataset(self, dataset):
        """Add persistent sample_id to dataset items"""
        # If dataset already has an ID field, use it
        if hasattr(dataset, 'data') and isinstance(dataset.data, list):
            for idx, item in enumerate(dataset.data):
                if 'sample_id' not in item:
                    # Use index as stable ID
                    item['sample_id'] = f"train_{idx}"
        elif hasattr(dataset, '__getitem__'):
            # For wrapped datasets, add sample_id via a wrapper
            original_getitem = dataset.__getitem__
            
            def getitem_with_id(idx):
                item = original_getitem(idx)
                if isinstance(item, dict) and 'sample_id' not in item:
                    item['sample_id'] = f"train_{idx}"
                return item
            
            dataset.__getitem__ = getitem_with_id
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        if not hasattr(train_dataset, 'sample_ids_added'):
            self._add_sample_ids_to_dataset(train_dataset)
            train_dataset.sample_ids_added = True
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        ...
        # )
        reward_model_keys = (
            set({"data_source", "reward_model", "extra_info", "uid", "prompt"}) & batch.non_tensor_batch.keys()
        )
    ...
    def _validate(self):
        ...
        for test_data in tqdm(self.val_dataloader):
            ...
            # print("Computing model attentions in validate...")
            # free up GPU memory before computing attentions
            ...
            del test_batch, test_gen_batch, test_gen_batch_padded, test_output_gen_batch_padded, test_output_gen_batch
            torch.cuda.empty_cache()
            gc.collect()
    ...
    def fit(self):
        ...
        if self.config.trainer.spo.enable:
            ...
            print(f"[DEBUG] Initializing SPO statistics from training dataset...")
            print(f"[DEBUG] Training dataset size: {len(self.train_dataset)}")
            
            # Iterate through the training dataset to create prompt mappings
            for idx in tqdm(range(len(self.train_dataset))):
                ...
                # Extract only the prompt part (before assistant response)
                before, separator, _ = prompt_text.partition("\nassistant\n")
                if separator:
                    prompt_key = before + separator
                else:
                    prompt_key = prompt_text
                prompt2scores[prompt_key] = [0]
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                # if self.config.trainer.spo.enable:
                #     # --- PARAMETERS ---
                #     EXPLORATION_EPSILON = 0.05  # Your new hyperparameter
                ...
                # print(f"[DEBUG] batch keys: {batch.batch}, {batch.non_tensor_batch.keys()}, {batch.meta_info.keys()}")
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # print("Computing model attentions...")
                    # free up GPU memory before computing attentions
                    torch.cuda.empty_cache()
                    if self.global_steps > 50 and self.config.trainer.get("enable_attention", False):
                        self.actor_rollout_wg.set_tokenizer(self.tokenizer)
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            # Convert summed rewards to python floats and group by unique uid
                            scores = reward_tensor.sum(dim=-1).detach().cpu().tolist()
                            index = batch.non_tensor_batch["uid"]
                            id2score = defaultdict(list)
                            for i, uid in enumerate(index):
                                id2score[str(uid)].append(int(scores[i]))

                            # Compute ratio of prompts where all repeated samples are all-zero or all-one
                            num_unique = len(id2score) if len(id2score) > 0 else 1
                            all_zeros = sum(1 for s_list in id2score.values() if all(s == 0 for s in s_list))
                            all_ones = sum(1 for s_list in id2score.values() if all(s == 1 for s in s_list))
                            metrics.update(
                                {
                                    "batch/solve_all": all_ones / num_unique,
                                    "batch/solve_none": all_zeros / num_unique,
                                }
                            )
                    ...
                    # print("#####################################")
                    # print(f"[DEBUG] batch non tensor keys is {batch.non_tensor_batch.keys()}")
                    # print(f"[DEBUG] batch is {batch.keys()}")
                    # micro_prompts = batch.non_tensor_batch.get("question", None)
                    # micro_prompts = [_.strip() for _ in micro_prompts]
                    input_ids = batch.batch["input_ids"]
                    # TODO: Can we keep special tokens except for padding tokens?
                    decoded_prompts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in input_ids]
                    micro_prompts = []
                    for prompt in decoded_prompts:
                        before, separator, _ = prompt.partition("\nassistant\n")
                        if separator:  # Only include if "\nassistant\n" exists
                            micro_prompts.append(before + separator)
                        else:
                            micro_prompts.append(prompt)
                    del decoded_prompts, input_ids
                    ...
                    if self.config.trainer.spo.enable:
                        ...
                        if self.config.trainer.spo.no_baseline:
                            advantages = r
                        else:
                            advantages = r - p_hats
                    ...
                    if rollout_data_dir:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            if "rollout_type" in batch.non_tensor_batch:
                                reward_extra_infos_dict.setdefault(
                                    "rollout_type",
                                    batch.non_tensor_batch["rollout_type"].tolist(),
                                )
