...
class ActorConfig(BaseConfig):
    ...
    apply_sink_suppression: bool = False
    apply_early_weighted_stability: bool = False
    apply_hook_attention: bool = False

    # Reward shaping knobs (used only when the matching apply_* flag is on).
    # Defaults match the values previously hardcoded inside dp_actor.py.
    stability_reward_penalty: float = 0.02       # `penalty`     in compute_windowed_global_stability*
    stability_reward_scale: float = 0.1          # `scale`       in compute_windowed_global_stability*
    stability_reward_sensitivity: float = 5.0    # `sensitivity` in compute_windowed_global_stability*
    suppression_reward_beta: float = 0.5         # `beta`        in compute_sink_suppression_reward
    suppression_reward_scale: float = 1.0        # `scale`       in compute_sink_suppression_reward
