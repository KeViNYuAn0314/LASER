...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def set_tokenizer(self, tokenizer):
        self.actor.tokenizer = tokenizer
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                _attn_kwargs = dict(
                    ...
                    apply_sink_suppression=self.config.actor.get("apply_sink_suppression", False),
                    apply_early_weighted_stability=self.config.actor.get("apply_early_weighted_stability", False),
                    stability_reward_penalty=self.config.actor.get("stability_reward_penalty", 0.015),
                    stability_reward_scale=self.config.actor.get("stability_reward_scale", 0.1),
                    stability_reward_sensitivity=self.config.actor.get("stability_reward_sensitivity", 5.0),
                    suppression_reward_beta=self.config.actor.get("suppression_reward_beta", 0.5),
                    suppression_reward_scale=self.config.actor.get("suppression_reward_scale", 1.0),
                )
                if self.config.actor.get("apply_hook_attention", False):
                    attention_scores, suppression_attention_scores = self.actor.generate_attentions_experimental_hooked(**_attn_kwargs)
                else:
                    attention_scores, suppression_attention_scores = self.actor.generate_attentions_experimental(**_attn_kwargs)
